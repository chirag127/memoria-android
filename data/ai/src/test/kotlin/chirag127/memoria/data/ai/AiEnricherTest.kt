...
class AiEnricherTest {
    ...
    private fun providerReturning(json: String) = object : LlmProvider {
        override val config = ProviderConfig(
            id = "fake", displayName = "Fake", baseUrl = "http://x/", apiKeyAlias = null,
            defaultModel = "m", caps = setOf(Modality.TEXT), tier = CostTier.FREE, priority = 0,
        )
    ...
    @Test
    fun `parses well-formed extraction json`() = runTest {
        val json = """{"title":"Rust talk","type":"youtube","tags":["rust"],
            "summary":"good","entities":[{"kind":"concept","name":"ownership","canonical":"Rust Ownership"}],
            "tasks":[],"links":["Rust Ownership"]}"""
        val out = AiEnricher(AiRouter(listOf(providerReturning(json)))).enrich("watched a rust video")
    ...
    @Test
    fun `falls back to title-from-first-line when ai returns junk`() = runTest {
        val out = AiEnricher(AiRouter(listOf(providerReturning("not json at all"))))
    ...
    @Test
    fun `falls back when no provider succeeds`() = runTest {
        val failing = object : LlmProvider {
            override val config = ProviderConfig("f", "F", "http://x/", null, "m", caps = setOf(Modality.TEXT), tier = CostTier.FREE, priority = 0)
            override fun supports(task: TaskKind) = true
